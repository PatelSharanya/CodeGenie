...
function activate(context) {
    ...
    let serverUrl = vscode.workspace.getConfiguration().get(CONFIG_SERVER_URL) || '';
    const autoConnect = vscode.workspace.getConfiguration().get(CONFIG_AUTOCONNECT) || false;
    // Register command to configure server
    context.subscriptions.push(vscode.commands.registerCommand('codegenie.configureServer', async () => {
    ...
    async function testConnection(url) {
        try {
            ...
            const response = await (0, cross_fetch_1.default)(`${url}/connection_info`, {
                method: 'GET',
    ...
    let optimizeDisposable = vscode.commands.registerCommand('codegenie.optimize', async () => {
        ...
        try {
            ...
            await vscode.window.withProgress({
                ...
                const response = await (0, cross_fetch_1.default)(`${serverUrl}/optimize`, {
                    method: 'POST',
                    headers: {
                ...
                const selection = await vscode.window.showInformationMessage("Code optimization complete. Apply changes?", { modal: true }, "Apply", "Cancel");
                if (selection === "Apply") {
                    progress.report({ message: "Applying changes..." });
                    // Apply the optimized code
                    await editor.edit((editBuilder) => {
                        const fullRange = new vscode.Range(editor.document.positionAt(0), editor.document.positionAt(originalCode.length));
    ...
    let debugDisposable = vscode.commands.registerCommand('codegenie.debugCode', async () => {
        ...
        try {
            ...
            vscode.window.withProgress({
                ...
                progress.report({ message: "Analyzing code..." });
                const response = await (0, cross_fetch_1.default)(`${serverUrl}/debug`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
    ...
    context.subscriptions.push(startDisposable, completeDisposable, fillMiddleDisposable, optimizeDisposable, debugDisposable);
    // Debounced functions
    ...
    async function getCompletion(text) {
        ...
        try {
            ...
            const response = await (0, cross_fetch_1.default)(`${serverUrl}/complete`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    async function handleCompletion(editor) {
        if (isProcessing || !ensureConnected())
            return;
        try {
            isProcessing = true;
            updateStatusBar("$(sync~spin) Generating completion...");
    ...
    async function getGhostCompletion(text) {
        if (!ensureConnected())
            return '';
        try {
            // Show status bar for ghost completion
            updateStatusBar("$(sync~spin) Generating suggestion...");
            const res = await (0, cross_fetch_1.default)(`${serverUrl}/hf-complete`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codegenie.acceptGhostSuggestion', () => {
        const editor = vscode.window.activeTextEditor;
    ...
    context.subscriptions.push(vscode.workspace.onDidChangeTextDocument(async (event) => {
        const editor = vscode.window.activeTextEditor;
        if (!editor || event.document !== editor.document || !isConnected)
            return;
    ...
    async function getMiddleFill(text) {
        if (!ensureConnected())
            return '';
        try {
            updateStatusBar("$(sync~spin) Generating middle fill...");
            const response = await (0, cross_fetch_1.default)(`${serverUrl}/fill_in_the_middle`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ text }),
            });
